...
// Function to generate a random price between 5 and 8000
...
const EnrollForm = ({ course, onClose }) => {
    ...
    const handleSubmit = (e) => {
        ...
        if (paymentMethod === 'upi') {
            paymentDetails = `Payment via UPI ID: ${upiId}`;
        } else if (paymentMethod === 'creditCard') {
            paymentDetails = `Payment via Credit/Debit Card: ${cardNumber}`;
        } else if (paymentMethod === 'paypal') {
            paymentDetails = `Payment via PayPal email: ${paypalEmail}`;
        }

        alert(`Successfully enrolled in ${course.title} for ₹${course.price}! ${paymentDetails} is successful.`);
        ...
        <div className="bg-white rounded-lg shadow-lg hover:shadow-xl transition-all duration-300 ease-in-out transform hover:scale-105 flex flex-col">
            ...
            <div className="p-6 pt-0">
                <button 
                    ...
                    className="bg-green-600 text-white py-2 px-6 rounded-full w-full hover:bg-green-700 transition duration-300"
                >
